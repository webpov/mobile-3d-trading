...
export default function LinkGridStage({children}:{children:ReactNode}) {
    ...
    return (<>
        <div className="tx-white flex-wrap tx-altfont-1 gap-1 tx-lx ">
            {!!loadingType && <>
            ...
            {!loadingType && gameTypes.map((aGameType:string, index: number) => {
                ...
                return (<div key={index} className="w-30">
                    <div  
                        className={`h-300px tx-white  nodeco  flex-col flex-justify-start pos-rel ${!lookupGameType.disabled ? "bg-w-20" : "bg-w-10"}`}
                    >
                        <div className="Q_xs_md tx-lgx pt-4 ">{aGameType.toUpperCase()}</div>
                        ...
                        <div className=" tx-bold-8 opaci-50 tx-ls-2 tx-center pt-1">
                            <div className="Q_xs tx-md ">{lookupGameType.name}</div>
