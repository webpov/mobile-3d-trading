...
export default function StageContainer({children}:{children:ReactNode}) {
  ...
  return (
    <div className="flex-col tx-altfont-1  ">
      <div className="pos-abs pa-8 bottom-0 mb-100 mr-8 right-0 flex-col bg-w-50 bord-r-25 z-500">
        <div>
          {parseInt(`${(dateNow - startRotationTime) / 1000}`)}
        </div>
        ...
        <div className="opaci-chov--50">
          ...
          {typeof window !== 'undefined'  &&
            <div className="flex gap-1 opaci-50 tx-lg">
              <h1>Goal:</h1>
              <h1>{ LS_maxScore }</h1>
            </div>
          }
      ...
      <Canvas style={{width:"100vw",height:"100vh"}} shadows camera={{fov:40,position:[3,0,0]}}
